...
use crate::{
    api::{
        api_call_get_media, api_call_media_add_attachment, api_call_media_remove_attachment,
        api_call_media_rename_attachment,
    },
    commands::logout::do_logout,
    models::MediaRenameAttachmentBody,
...
pub async fn run_cmd_upload_media_attachment(
    ...
    match api_res {
        Ok(att) => {
            ...
            let att_id = att.id;
            eprintln!("Attachment ID: {att_id}");
            let att_name = att.name;
            eprintln!(
                "Successfully uploaded new attachment file for #{media_id_param}: {att_name}"
            );
        }
        Err(e) => {
            if logout_after_operation {
...
pub async fn run_cmd_delete_media_attachment(
    global_opts: CommandGlobalOptions,
    media: String,
    ...
    let attachment_id = match parse_identifier(&attachment_id) {
        Ok(id) => id,
        Err(_) => {
            if logout_after_operation {
                let logout_res = do_logout(&global_opts, &vault_url).await;
            ...
            eprintln!("Invalid attachment identifier specified.");
            process::exit(1);
        }
    };
    ...
    match api_res {
        Ok(_) => {
            eprintln!(
                "Successfully removed attachment file #{attachment_id} from #{media_id_param}"
            );
        }
        Err(e) => {
            if logout_after_operation {
...
pub async fn run_cmd_rename_media_attachment(
    global_opts: CommandGlobalOptions,
    media: String,
    attachment_id: String,
    name: String,
    ...
    match api_res {
        Ok(_) => {
            eprintln!("Successfully renamed attachment file #{attachment_id} from #{media_id_param}: {name}");
        }
        Err(e) => {
            if logout_after_operation {
                let logout_res = do_logout(&global_opts, &vault_url).await;
